...
async function generateImportReport() {
  ...
  try {
    ...
    csvRecords.forEach((record, index) => {
      ...
      if (!email || email === "") {
        ...
        if (isFirstOccurrence && dbEmailMap.has(email)) {
          ...
          details = `Email appears ${emailCounts.get(
            email
          )} times in CSV, but import failed`;
          ...
          details = `${
            index - emailFirstOccurrence.get(email)
          } occurrence(s) after first`;
        ...
        details = `Imported on ${
          dbRecord.createdAt.toISOString().split("T")[0]
        }`;
        actionNeeded = "None";
        stats.imported++;
      } else {
    ...
    console.log(
      `\nSuccess rate: ${((stats.imported / csvRecords.length) * 100).toFixed(
        1
      )}%`
    );
    ...
    const failedRecords = reportData.filter(
      (row) => row.status === "FAILED_IMPORT"
    );
    if (failedRecords.length > 0) {
      ...
      failedRecords.slice(0, 10).forEach((record, i) => {
        console.log(
          `${i + 1}. ${record.firstName} ${record.lastName} (${record.email})`
        );
      });
      if (failedRecords.length > 10) {
        console.log(
          `... and ${failedRecords.length - 10} more (see CSV for full list)`
        );
      }
    }
