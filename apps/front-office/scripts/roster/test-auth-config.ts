...
async function testAuthConfiguration() {
  ...
  try {
    ...
    console.log(
      `   Database configured: ${auth.options.database ? "Yes" : "No"}`
    );
    ...
    const hasOrganizationPlugin = plugins.some(
      (plugin: any) =>
    ...
    console.log(
      `   Cookie cache enabled: ${sessionConfig?.cookieCache?.enabled || false}`
    );
    console.log(
      `   Cache max age: ${
        sessionConfig?.cookieCache?.maxAge || "Not set"
      } seconds`
    );
    ...
    if (userFields) {
      ...
      knownFields.forEach((field) => {
        ...
        if (fieldConfig) {
          console.log(
            `   ✓ ${field} (${fieldConfig.type}, required: ${fieldConfig.required})`
          );
        }
      });
    }
