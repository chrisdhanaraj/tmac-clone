...
import {
  ...
  GearSize,
  TennisRanking,
} from "../../app/generated/prisma/index.ts";
...
function findClosestMatch(value, enumValues) {
  ...
  const partialMatch = enumValues.find(
    (e) =>
      e.toLowerCase().includes(lowerValue) ||
      lowerValue.includes(e.toLowerCase())
  );
...
async function validateRecord(record) {
    ...
    if (!result.isValid) {
      ...
      if (result.suggestion) {
        validation.suggestions.push(
          `${field.name}: Try '${result.suggestion}' instead of '${field.value}'`
        );
      }
    }
  }
  ...
  if (
    !validation.sanitizedRecord.firstName &&
    !validation.sanitizedRecord.lastName
  ) {
...
async function retryFailedRecords() {
  ...
  try {
    ...
    const existingEmails = new Set(
      existingRecords.map((r) => r.email?.toLowerCase().trim()).filter(Boolean)
    );
    ...
    for (let i = 0; i < missingRecords.length; i++) {
      ...
      console.log(
        `🔄 [${recordNum}/${missingRecords.length}] Processing: ${record.firstName} ${record.lastName} (${record.email})`
      );
    ...
    const errorReportPath = join(
      __dirname,
      "../data/failed_records_detailed_analysis.json"
    );
    writeFileSync(
      errorReportPath,
      JSON.stringify(
    ...
    console.log(
      `Success rate: ${((results.successful / results.attempted) * 100).toFixed(
        1
      )}%`
    );
    console.log(
    ...
    if (results.failed > 0) {
      ...
      console.log(
        "Check the detailed analysis file for specific issues and suggestions."
      );
    }
  } catch (error) {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  retryFailedRecords().catch(console.error);
}
