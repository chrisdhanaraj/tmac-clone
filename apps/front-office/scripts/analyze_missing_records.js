...
async function analyzeImportGap() {
    ...
    try {
        ...
        console.log(`📊 Gap: ${csvRecords.length - dbRecords.length} missing records\n`);
        ...
        missingEmails.slice(0, 20).forEach((email, i) => {
            const csvRecord = csvRecords.find(r => r.email?.toLowerCase().trim() === email);
            console.log(`${i + 1}. ${csvRecord?.firstName || 'null'} ${csvRecord?.lastName || 'null'} (${email})`);
        });
        ...
        const recordsWithoutEmail = csvRecords.filter(r => !r.email || r.email.trim() === '');
        console.log(`\n📭 Records without email: ${recordsWithoutEmail.length}`);
        
        // Calculate expected vs actual
        const totalDuplicates = duplicateEmails.reduce((sum, [, count]) => sum + (count - 1), 0);
        const expectedImports = csvRecords.length - recordsWithoutEmail.length - totalDuplicates;
