...
async function checkImportCount() {
    try {
        ...
        console.log(`👤 Records with userId (linked to user account): ${withUserId}`);
        ...
        sampleRecords.forEach((record, index) => {
            console.log(`${index + 1}. ${record.firstName || 'null'} ${record.lastName || 'null'} (${record.email}) - Created: ${record.createdAt}`);
        });
