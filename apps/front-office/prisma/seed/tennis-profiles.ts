...
function validateEthnicity(value: string): Ethnicity | null {
  if (!value) return null;
  return Object.values(Ethnicity).includes(value as Ethnicity)
    ? (value as Ethnicity)
    : null;
}
...
function validateDistrict(value: string): District | null {
  if (!value) return null;
  return Object.values(District).includes(value as District)
    ? (value as District)
    : null;
}
...
function validateTmacGearPreference(value: string): TmacGearPreference | null {
  if (!value) return null;
  return Object.values(TmacGearPreference).includes(value as TmacGearPreference)
    ? (value as TmacGearPreference)
    : null;
}

function validateGearSize(value: string): GearSize | null {
  if (!value) return null;
  return Object.values(GearSize).includes(value as GearSize)
    ? (value as GearSize)
    : null;
}
...
function validateTennisRanking(value: string): TennisRanking | null {
  if (!value) return null;
  return Object.values(TennisRanking).includes(value as TennisRanking)
    ? (value as TennisRanking)
    : null;
}
...
async function importBatch(
  ...
  for (const record of records) {
    try {
      ...
      if (options.dryRun) {
        console.log(
          `   📝 [DRY RUN] Would import: ${transformedRecord.firstName} ${transformedRecord.lastName} (${transformedRecord.email})`
        );
        results.imported++;
      } else {
        // Create the profile
        ...
        console.log(
          `   ✅ Imported: ${transformedRecord.firstName} ${transformedRecord.lastName} (${transformedRecord.email})`
        );
        results.imported++;
...
async function importTennisProfiles(
  options: ImportOptions = {}
): Promise<ImportStats> {
  ...
  try {
    ...
    const existingEmails = skipDuplicates
      ? await checkForDuplicates(records)
      : new Set<string>();
